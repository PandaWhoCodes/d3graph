...
def d3graph(
    ...
    edge_distance=None,
    directed=False,
    edge_distance_minmax=[None, None],
    title="d3graph",
    slider=None,
    savepath=None,
    savename="index",
    ...
    [G, df] = adjmat2G(
        adjmat,
        df,
        config["edge_distance_minmax"][0],
        config["edge_distance_minmax"][1],
        config["edge_width"],
    )
    # Make slider
...
def _write_index_html(config, json_data):
    ...
    with open(config["savepath"], "w", encoding="utf-8") as index_file:
        # print("Writing %s" % index_file.absolute())
        index_file.write(index_template.render(content))
...
def _node_config(
    ...
    # Sanity checks
    assert (
        len(node_size) == nodecount
    ), "Node size must be of same length as the number of nodes"
    assert (
        len(node_size_edge) == nodecount
    ), "Node size edge must be of same length as the number of nodes"
    assert (
        len(node_color) == nodecount
    ), "Node color must be of same length as the number of nodes"
    assert (
        len(node_color_edge) == nodecount
    ), "Node color edge must be of same length as the number of nodes"
    assert (
        len(node_name) == nodecount
    ), "Node label must be of same length as the number of nodes"
...
def adjmat2G(
    adjmat, df, edge_distance_min=None, edge_distance_max=None, edge_width=None
):
    """Convert adjacency matrix to graph.
    ...
    if isinstance(edge_width, type(None)):
        if len(np.unique(adjmat["weight"].values.reshape(-1, 1))) > 2:
            adjmat["edge_width"] = _normalize_size(
                adjmat["weight"].values.reshape(-1, 1), 1, 20
            )
        else:
            edge_width = 1
    ...
    if not isinstance(edge_distance_min, type(None)):
        adjmat["edge_weight"] = _normalize_size(
            adjmat["weight"].values.reshape(-1, 1), edge_distance_min, edge_distance_max
        )
    else:
    ...
    A = pd.concat(
        [
            pd.DataFrame(adjmat[["target", "target_label"]].values),
            pd.DataFrame(adjmat[["source", "source_label"]].values),
        ],
        axis=0,
    )
...
def _set_configurations(
    ...
    config["d3_script"] = os.path.abspath(
        os.path.join(curpath, "d3js/d3graphscript.js")
    )
...
def _do_checks(adjmat):
    ...
    if "numpy" in str(type(adjmat)):
        adjmat = pd.DataFrame(
            index=range(0, adjmat.shape[0]),
            data=adjmat,
            columns=range(0, adjmat.shape[0]),
        )
    ...
    assert np.all(
        adjmat.columns == adjmat.index.values
    ), "adjmat columns and index must have the same identifiers"
    return adjmat
